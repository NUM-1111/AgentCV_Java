...
> ⚠️ **状态**：本文档部分内容过时（引用已删除的 `RewriteCoordinatorService`、`RewriteReport`，循环轮数为旧版3轮）。当前架构已重构为 `ResumeOptimizationService` 统一编排三阶段管线（评分→改写→审查），循环上限改为2轮（`DEFAULT_MAX_ROUNDS = 2`）。
>
> **最新架构文档**：见 `05-面试难点合集-Critic审查质量保障.md` 的方向2a/2b（Writer/Critic 完整工作链路图）。

---

## 1. 问题背景：为什么改写接口是幻觉重灾区

LLM 在单次生成中面临两个矛盾目标：写得更好（创意生成）和不捏造（事实约束）。单次调用时模型倾向于"化腐朽为神奇"——自行填入不存在的量化数据和技术栈，因为没有外部审查机制，生成结果的真实性完全依赖模型自律。

当前解决方案：三 Agent 职责分离——评分引擎解读 JD、Writer 在事实边界内改写、Critic 做独立事实核查。详见 `05-面试难点合集-Critic审查质量保障.md`。
